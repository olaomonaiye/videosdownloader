...
export default function EditStaticPage() {
  ...
  useEffect(() => { adminApi.getPage(id as string).then(setForm); }, [id]);

  const handleSubmit = async () => {
    setLoading(true);
    try { await adminApi.updatePage(id as string, form); router.push('/admin/pages'); } catch(e: any) { alert(e.response?.data?.error?.message || 'Failed'); }
    setLoading(false);
  };
  return (
    <div><div className="flex items-center gap-4 mb-6"><Link href="/admin/pages" className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-900"><ArrowLeft className="h-5 w-5" /></Link><h1 className="text-2xl font-bold text-slate-900 dark:text-white">Edit Page</h1></div>
