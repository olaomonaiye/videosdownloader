...
export default function EditPostPage() {
  ...
  const [form, setForm] = useState<any>({ title: '', content: '', excerpt: '', status: 'DRAFT', categoryIds: [], tagNames: [], metaTitle: '', metaDescription: '', thumbnailUrl: '' });

  useEffect(() => {
    Promise.all([adminApi.getPost(id as string), adminApi.getCategories()]).then(([post, cats]) => {
      setForm({ ...post, categoryIds: post.categories?.map((c: any) => c.id) || [], tagNames: post.tags?.map((t: any) => t.name) || [] });
      setCategories(cats);
      if (post.thumbnailUrl) setThumbPreview(post.thumbnailUrl);
      if (post.excerpt) setExcerptTouched(true);
    });
  }, [id]);

  // Auto-fill excerpt only when creating fresh content without manual excerpt
  useEffect(() => {
    if (!excerptTouched && form.content) {
      ...
      if (plain.length > 0) {
        ...
        setForm((f: any) => ({ ...f, excerpt: autoExcerpt }));
      }
    }
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      setThumbPreview(url);
      setForm((f: any) => ({ ...f, thumbnailUrl: url }));
    } catch {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try { await adminApi.updatePost(id as string, form); router.push('/admin/posts'); }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div><label className="block text-sm font-medium mb-1">Title</label><input value={form.title || ''} onChange={e => setForm({...form, title: e.target.value})} className="input-field" /></div>
        ...
        <div className="space-y-4">
          <div className="card">
            ...
            <select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="input-field mb-3"><option value="DRAFT">Draft</option><option value="PUBLISHED">Published</option><option value="ARCHIVED">Archived</option></select>
            <button onClick={handleSubmit} disabled={loading} className="btn-primary w-full"><Save className="h-4 w-4" /> {loading ? 'Saving...' : 'Update Post'}</button>
          </div>
          <div className="card">
            ...
            {categories.map(c => (
              <label key={c.id} className="flex items-center gap-2 py-1 text-sm">
                <input type="checkbox" checked={(form.categoryIds || []).includes(c.id)}
                  onChange={e => setForm({...form, categoryIds: e.target.checked ? [...(form.categoryIds||[]), c.id] : (form.categoryIds||[]).filter((x: string) => x !== c.id)})} />
                {c.name}
              </label>
            ))}
          </div>
          ...
          <div className="card"><h3 className="font-semibold mb-3">SEO</h3>
